...
func RegisterGatewayRoutes(
	...
	imagesHandler := func(c *gin.Context) {
		switch getGroupPlatform(c) {
		case service.PlatformOpenAI, service.PlatformMiniMax:
	...
	speechHandler := func(c *gin.Context) {
		...
		c.JSON(http.StatusNotFound, gin.H{
			"error": gin.H{
				"type":    "not_found_error",
				"message": "Speech API is not supported for this platform",
			},
		})
	}
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST("/audio/speech", textBodyLimit, speechHandler)
	...
	r.POST("/audio/speech", textBodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, speechHandler)
